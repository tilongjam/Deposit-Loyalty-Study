...
for kind in customer_type:
    ...
    with pd.ExcelWriter(f'{kind}_population_data.xlsx') as writer:
        ...
        for branch in branch_list:
            ...
            for product_type in summary_age.index:
                ...
                for age_group in summary_age.columns:
                    ...
                    if summary_age.at[product_type, 'Grand Total'] == 1 and summary_age.loc[product_type].drop(
                        ...
                        if len(population_data_df) == 0:
                            population_data_df = population_data
                            print("shit empty dawg")
                        else:
                            pd.concat([population_data_df, population_data])
                            print('im filling u up')
                        ...
                        if not filtered_rows.empty and i ==0:
                            ...
                            if not population_data.empty:
                                ...
                                if len(sample_data_df) == 0:
                                    sample_data_df = sample_data
                                else:
                                    pd.concat([sample_data_df, sample_data])
                        ...
                        population_data = corp_master_data[(corp_master_data['PRODUCT_TYPE'] == product_type) &
                                                           ...
                                                           (corp_master_data['Geographical Location'] == branch)]
                        population_data_df2 = population_data
            ...
            if extracted_rows:
                ...
                full_pop_data = pd.concat([population_data_df, population_data_df2])
                full_samp_data = pd.concat([sample_data_df, sample_data_df2])
                ...
                # sample_data_df = pd.DataFrame(sample_data_df)

                filepath = 'population_data_for_avg_month_end_bal_pivot.csv'
                full_pop_data.to_csv(filepath)

                filepath = 'sample_data_for_avg_month_end_bal_pivot.csv'
                full_samp_data.to_csv(filepath)
                ...
                # #
                # avg_bal_1_yr_sample.to_excel(writer, sheet_name=branch, startrow=summary_age.shape[0] + 10)
                ...
                full_pop_data.to_excel(writer, sheet_name=branch, startrow=avg_bal_1_yr.shape[0] + 15, index=False)
...
# filepath = 'population_data.csv'
# population_data_df.to_csv(filepath)
# filepath = 'population_data_sampled.csv'
# sample_data_df.to_csv(filepath)
