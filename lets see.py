...
df = pd.read_csv(r"C:\Users\tilongjam.ext\PycharmProjects\Data Analysis w CIF ID\subset_and_master_data.csv",
...
df['MONTHS'] = df['MONTHS'].apply(safe_to_datetime)

# Check for any NaT values that indicate conversion issues
if df['MONTHS'].isna().any():
    print("Warning: Some dates in 'MONTHS' could not be converted.")
...
def calculate_month_end_balance(df, product_type, age_group):
    ...
    df = df.groupby([product_type, age_group]).max('Last_Date')
    show(df)

    filtered_df = df[(df['Product_Types'] == product_type) & (df['Age_Group'] == age_group)]

    if filtered_df.empty:
        print(f"No data found for Product Type: {product_type}, Age Group: {age_group}")
    ...
    if pd.isna(last_date):
        print(f"No valid Last_Date found for Product Type: {product_type}, Age Group: {age_group}")
    ...
    if last_year_df.empty:
        print(f"No data for the last year found for Product Type: {product_type}, Age Group: {age_group}")
        return pd.DataFrame()  # Return empty DataFrame if no data in the last year
